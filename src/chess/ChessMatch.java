...
public class ChessMatch {
    ...
    private ChessPiece king(Color color) {
        ...
        ChessPiece king = (ChessPiece) list.stream().filter(x -> x instanceof King).findFirst().orElse(null);
        if (king == null) {
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        if (targetPiece != null) {
            piecesOnTheBoard.remove(targetPiece);
        ...
        if (piece instanceof Pawn) {
            if (source.getColumn() != target.getColumn() && targetPiece == null) {
    ...
    private void undoMove(Position source, Position target, Piece targetPiece) {
        ChessPiece piece = (ChessPiece) board.removePiece(target);
        piece.decreaseMoveCount();
        board.placePiece(piece, source);

        if (targetPiece != null) {
            board.placePiece(targetPiece, target);
            capturedPieces.remove(targetPiece);
            piecesOnTheBoard.add(targetPiece);

            // special move en passant
            if (piece instanceof Pawn && source.getColumn() != target.getColumn() && targetPiece == enPassantVulnerable) {
                ...
                board.placePiece(targetPiece, pawnPosition);
            }
        }
    ...
    public boolean[][] possibleMoves(ChessPosition sourcePosition) {
        ...
        boolean[][] mat = board.piece(p).possibleMoves();
        for (int i = 0; i < board.getRows(); i++) {
            for (int j = 0; j < board.getColumns(); j++) {
                if (mat[i][j]) {
                    Position target = new Position(i, j);
                    Piece capturedPiece = makeMove(p, target);
                    boolean testCheck = testCheck(currentPlayer);
                    undoMove(p, target, capturedPiece);
                    if (testCheck) {
                        mat[i][j] = false;
                    }
                }
